...
public class GameRenderer implements Disposable {
    ...
    public GameRenderer(BallCage game, SpriteBatch batch, AssetManager assetManager, GameController controller, int stage) {
        this.game = game;
        this.batch = batch;
    ...
    private void init(int stage) {
        ...
        blocks = controller.getGameBloks(stage);
        floors = controller.getGameFloors(stage);
        balls = controller.getGameBalls(stage);
    ...
    public void render(float delta) {
        ...
        if(!finishStage && controller.controlFinishStage(balls, floors)) {
            stars = controller.convertBalls(balls);
            finishStage = true;
    ...
    private Actor createStageUi() {
        ...
        ImageButton next = new ImageButton(uiskin, "next");
        ...
        if(gameOver)
            ...
            if(GameConfig.level < stage + 1) {
                GameConfig.level = stage + 1;
                GameConfig.PREFS.putInteger(GameConfig.STAGE_KEY, GameConfig.level);
    ...
    private void renderGamePlay() {
        ...
        if(effectFont != null) {
            glyphLayout.setText(effectFont, "STAGE: " + stage);
            effectFont.draw(batch, "[#960019]" + "STAGE: " + stage,  GameConfig.HUD_WIDTH / 2 - glyphLayout.width / 2,
